...
- **QA-C2 — Mobile + content polish** (pending): apply-modal mobile padding, privacy-page top padding mobile, remove "Rezultatet Flasin Vetë" section (all viewports), employer company description min 400 chars.
- **QA-C3 — Email-exists onBlur** (pending): new `GET /auth/check-email` endpoint + `useEmailAvailability` hook wired into employer + jobseeker signup with inline red error.
- **QA-C4 — Paysera scaffolding** (pending): pricing constants 35/49 EUR, `/payment/job/:id` paywall page (placeholder "coming soon" button), 402 handler on EmployerDashboard "View Candidates", `payseraService.js` stub.
- **QA-C5 — Perf quick wins** (pending): `optimizedCloudinaryUrl` helper + `loading="lazy"` + `React.memo` on JobCard/SimilarJobs + `useMemo` for `getMergedJobs`.
