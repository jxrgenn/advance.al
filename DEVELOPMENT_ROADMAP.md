...
**2026-06-08 — Live SEO/GEO audit + fixes (pre-launch, site ~2mo live).** Full live audit of deployed advance.al (40-agent workflow + manual verification against the live site). **Healthy:** bot-prerender/dynamic-rendering fires correctly for Googlebot/GPTBot/ClaudeBot/PerplexityBot (real `<h1>`+JSON-LD); robots.txt/sitemap.xml/llms.txt/IndexNow-key all live & byte-identical to repo (zero deploy drift); JobPosting schema fully Google-for-Jobs eligible. **GSC is DNS-verified** (`dig TXT advance.al` → `google-site-verification=aOdlMc0qCb-Y4Y5jE4VCmPk2UMbl6waBZRJQTL0taLE`) and **sitemap was submitted & is healthy** (GSC Sitemaps: Success, 16 discovered) — earlier roadmap implication that GSC/verification was unset is WRONG. **Indexing state:** 3 indexed, 13 not — 9 "Discovered – currently not indexed" (blog ×6 + /jobseekers + /register + /employer-register), 3 "Page with redirect" (benign www/http variants), 1 "Duplicate, Google chose different canonical" (/jobs — thin-content/homepage-overlap, resolves with real jobs). Root cause of non-indexing = new-domain low authority + thin content (no real jobs yet), NOT a config bug.
- **Fixes shipped (pending commit):** (1) `/login`, `/register`, `/employer-register` (+ `/forgot-password`, `/reset-password`, `/unsubscribe`, `/preferences`, `/report-user`) now **NOINDEX** — `frontend/api/seo.js` `NOINDEX_PATHS` set (was: catch-all served an indexable Organization shell with the DEFAULT homepage title → Google duplicate-clustered them against `/`) + `<SEO noindex>` on `Login.tsx` (both `/login` and `/register` renders) and `EmployerRegister.tsx`. (2) Removed those 3 thin auth URLs from `frontend/public/sitemap.xml` (16 → 13 content URLs). (3) `frontend/public/llms.txt` "14 industries" → **25 categories** (full list synced from `jobCategories.ts`; was omitting 11 incl. Shitës/e, Shërbim Klienti, Gastronomi, Logjistikë, Bujqësi, Punë Krahu, Pastrim, Bukuri, Siguri, Media, Drejtësi). (4) Removed dead Netlify `frontend/public/_redirects` (Vercel ignores it). Build clean (4.02s); prerender noindex verified offline (auth→`noindex,nofollow`, content→`index,follow`); sitemap valid XML.
- **STILL TODO (user/ops side, not code):** in GSC submit sitemap re-fetch + **Request Indexing** for blog ×6 + `/jobseekers`; **verify Bing Webmaster** (no Bing verification yet — only `/register` indexed there); create LinkedIn/FB/IG profiles + populate `Organization.sameAs` (empty `[]` in `seo-templates.js:41` AND `index.html`); make `www`→apex **307→308**; prerender `Vary: User-Agent` disables Vercel edge cache so `s-maxage` is dead (every bot hit = live Render call — needs a careful, separately-tested fix); `favicon.ico` is a renamed 102KB JPEG (not a real .ico); **remove junk `asdfadsfdsaf`/JXXS test job before launch (DEFERRED per user — still testing)**.
