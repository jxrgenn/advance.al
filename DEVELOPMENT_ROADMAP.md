...
- **QA-E2 — Paysera live** (pending): payseraService real impl (md5(data+sign_pass) ss1 signing per spec v1.6), payments route file (initiate, callback, dev-only fake-success), PaymentJobPosting tier selector, PaymentSuccess/Cancel/FakeSuccess pages.
- **QA-E3 — Payment tests** (pending): unit tests for signing math determinism, integration tests for ownership/status/signature/idempotency.
- **QA-E4 — Perf verify live** (pending): start dev server, inspect rendered JobCard img src for /upload/f_auto/, measure response sizes, document findings.
