...
- **QA-E4 — Perf verify (static portion)** (SHIPPED 2026-05-15): I cannot drive a real browser from this session, so live Network/Elements inspection is out of scope here — flagged explicitly rather than claiming success. STATIC verification performed: `optimizedCloudinaryUrl` helper logic tested against 9 representative URLs and all 9 produce expected output: plain Cloudinary URL gets `f_auto,q_auto,w_<W>,c_fill` injected; URL already containing `f_auto` is no-op; URL with only `q_auto` STILL gets `f_auto` injected (the D4 regex fix `/\/upload\/[^/]*\bf_auto\b[^/]*\//` is working); non-Cloudinary `/uploads/...` URLs pass through unchanged; external URLs pass through; null/undefined returns undefined cleanly; false-positive guard (`f_auto` appearing in a filename, not transform path) does NOT cause skip. Helper integrated in 3 call sites: `JobCard.tsx:203` (width:96 for logo), `PremiumJobsCarousel.tsx:132` (width:128), `Navigation.tsx:405` (avatar). `JobCard` wrapped in `memo(JobCard)` at line 251. `<img loading="lazy" decoding="async">` on logo. REMAINING (requires user to manually verify in browser): (a) rendered DOM `<img src>` actually contains `/upload/f_auto,q_auto,w_96,c_fill/`; (b) Network panel shows AVIF/WebP Content-Type (the `f_auto` benefit) and response size in the 5-30KB range, not 100-300KB; (c) React DevTools Profiler confirms JobCard nodes don't re-render under unrelated parent updates. Verification script provided to user in chat.
